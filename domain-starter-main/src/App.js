...
const tld = '.ninja';
const CONTRACT_ADDRESS = '0x44B732A0D902CDA916Fa07Cf44C4059a5c0C5a51';
...
const App = () => {
	...
	const mintDomain = async () => {
		console.log('mintDomain 00')
		// Don't run if the domain is empty
		if (!domain) { return }
		console.log('mintDomain 01')
		// Alert the user if the domain is too short
		if (domain.length < 3) {
			alert('Domain must be at least 3 characters long');
			return;
		}
		console.log('mintDomain 02')
		...
		try {
			...
			if (ethereum) {
				...
				let tx = await contract.register(domain, {value: ethers.utils.parseEther("0.001")});
	...
	const fetchMints = async () => {
		try {
			...
			if (ethereum) {
				...
				// const name0 = await contract.names(0);
				// console.log(name0);
